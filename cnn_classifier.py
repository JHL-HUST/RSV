...
def main():
    ...
    if args.do_train:
        ...
        train_examples, labels = read_text("%s/train" % task_name, args.data_dir)

        vocab, _ = load_dictionary(task_name, args.vocab_size, data_dir=args.data_dir)
        seqs, seqs_mask = text_encoder(train_examples, vocab, args.max_seq_length)
        ...
        # Prepare model
        pretrained_emb = load_pretrained_embedding(task_name, args.vocab_size, args.data_dir)
        model = CNNModel(num_labels, vocab, args.max_seq_length, device, pretrained_emb=pretrained_emb.T, freeze_emb=args.freeze_emb)
        model.init_weight()
        model.to(device)
